...
function generateRouter(data) {
   ...
   */
  let result = [];  
  const mapProfileObj = data.profile.filter((item) => ["GET", "POST", "PUT", "PATCH", "DELETE", "Route"].includes(item.name))
    .reduce((acc, item) => {
      acc[item.id] = item.name;
      return acc;
    }, {});
  const filteredData = data.package.filter(item => mapProfileObj[item.stereotype] === "Route");
  // Get endPathList can't start point
  let endPathList = [];
  ...
  filteredData.forEach(element => {
    ...
    if (!endPathList.includes(element.nameId)) {
      newPath.nameOfFile = `${element.name.toLowerCase().replace("route","")}.route.js`;
      let dependency = [];
      let resource = [];
      let methodName = [];
      let controllerName = [];
      ...
      // Recursive routePath until routePath = 0
          ...
          for (const elem of filteredData) {
            if (iterator.endPath == elem.nameId) {
              ...
              elem.operation.forEach(elem_opr => {
                ...
                path = `${currentPath}/${elem.name.toLowerCase().replace("route","")}${defineNewPath}`;
                controller = elem_opr.nameOpr;
                templateRoute = `\nrouter.${method.toLowerCase()}("/${path}", ${controller});`;
                content = templateRoute;
                // Insert Next Controller
                controllerName.push(controller);
                newPath.contentFile.push(content);
              });
              nextRoutePath(elem.routePath, path.toLowerCase().replace("route",""));
            }            
          }
        }
      }
      nextRoutePath(element.routePath, element.name.toLowerCase().replace("route",""));   
      let commentSection = `
/**
...
function generateModel(data) {
   ...
   */
  let result = [];
  const mapType = data.type.filter((item) => ["String", "Boolean", "Integer", "Float", "Date", "Id"].includes(item.name))
  .reduce((acc, item) => {
    acc[item.id] = item.name;
    return acc;
  }, {});
  const mapProfile = data.profile.filter((item) => ["Model", "Database"].includes(item.name))
    .reduce((acc, item) => {
      acc[item.id] = item.name;
      return acc;
    }, {});

  const filteredDataModel = data.package.filter(item => mapProfile[item.stereotype] === "Model");
  const idToNameMap = {};
  filteredDataModel.forEach(obj => {
    idToNameMap[obj.nameId] = obj.name;
  });
  const filteredDataDB = data.package.filter(item => mapProfile[item.stereotype] === "Database");
  ...
  if (isSQL) {
    filteredDataModel.forEach(element => {
      ...
      newModel.nameOfFile = `${element.name.toLowerCase().replace("model","")}.model.js`;
      let model = [];
      model.push(element.name);
      let relations = [];
      if (element.pairModel?.length) {
        for (const a of element.pairModel) {
          const filteredRelations = element.association.filter(item => item.assId === a.id2);
      ...
      class ${element.name.replace("Model","")} extends Model {}
        ${element.name.replace("Model","")}.init(
          ...
          },
          {
            ...
            tableName: '${element.name.toLowerCase().replace("model","")+"s"}',
            timestamps: false,
          }
        );
...
if (element.attribute?.length != 0) {
  ...
  class ${element.name.replace("Model","")} {
    constructor() {
      ...
      this.model = mongoose.model('${element.name.replace("Model","")}', this.schema);
    }
  }
...
function generateController(data) {
  let result = [];
  const mapProfile = data.profile.filter((item) => ["Controller"].includes(item.name))
  .reduce((acc, item) => {
    acc[item.id] = item.name;
    return acc;
  }, {});
  const filteredData = data.package.filter(item => mapProfile[item.stereotype] === "Controller");
  filteredData.forEach(element => {
    let newController = {
      nameOfFile: "",
      contentFile: [],
    };
    newController.nameOfFile = `${element.name.toLowerCase().replace("controller","")}.controller.js`;
    // Insert Class
    let className = [];
